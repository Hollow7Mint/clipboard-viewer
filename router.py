"""Clipboard Viewer — router for snapshot payloads."""
...
class ClipboardRouter:
    """Router for Clipboard Viewer snapshot payloads."""
...
def parse_snapshots(payload: str) -> List[Dict[str, Any]]:
    ...
    return [ClipboardRouter._coerce(item) for item in raw]


def snapshot_snapshot_to_str(
    record: Dict[str, Any], indent: Optional[int] = None
) -> str:
    """Convenience wrapper — serialise a Snapshot to a JSON string."""
    if indent is None:
        return ClipboardRouter.dumps(record)
