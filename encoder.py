"""Clipboard Viewer — encoder for clip payloads."""
...
def parse_clips(payload: str) -> List[Dict[str, Any]]:
    """Parse a JSON array of Clip payloads."""
    raw = json.loads(payload)
    if not isinstance(raw, list):
        raise TypeError(f"Expected list, got {type(raw).__name__}")
    return [ClipboardEncoder._coerce(item) for item in raw]


def export_clip_to_str(
    record: Dict[str, Any], indent: Optional[int] = None
) -> str:
    """Convenience wrapper — serialise a Clip to a JSON string."""
    if indent is None:
        return ClipboardEncoder.dumps(record)
