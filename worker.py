"""Clipboard Viewer — Entry repository."""
...
class ClipboardWorker:
    """Thin repository wrapper for Entry persistence in Clipboard Viewer."""

    TABLE = "entrys"

    def __init__(self, db: Any) -> None:
        self._db = db
        logger.debug("ClipboardWorker bound to %s", db)

    def insert(self, size_bytes: Any, index: Any, **kwargs: Any) -> str:
    ...
    def snapshot_by_source(
        ...
        """Fetch entrys filtered by *source*."""
    ...
    def bulk_insert(
        ...
        for rec in records:
            rec_id = self.insert(
                rec["size_bytes"], rec.get("index"),
                **{k: v for k, v in rec.items() if k not in ("size_bytes", "index")}
            )
            ids.append(rec_id)
